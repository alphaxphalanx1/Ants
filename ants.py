    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(self.ant, QueenAnt) and self.ant.true_queen:
                return
            elif self.ant.is_container:
                if isinstance(self.ant.contained_ant, QueenAnt) and self.ant.contained_ant.true_queen:
                    return
            # END Problem 13
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        tiles_forward = 0
        while curr != hive:
            if len(curr.bees) > 0 and tiles_forward <= self.max_range and tiles_forward >= self.min_range:
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 7
        "*** YOUR CODE HERE ***"
        bees_to_ninjitsu = self.place.bees[:]
        for bee in bees_to_ninjitsu:
            Insect.reduce_armor(bee, self.damage)
        # END Problem 7
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt has a large armor value."""
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        # BEGIN Problem 10
        "*** YOUR CODE HERE ***"
        bees_to_shoot = self.place.bees[:]
        for bee in bees_to_shoot:
            Insect.reduce_armor(bee, self.damage)
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is like an expensive ThrowerAnt that can be placed on Water."""
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.true_queen:
            ...
            while curr != None:
                if curr.ant != None:
                    if curr.ant.is_container == True and curr.ant.contained_ant != None:
                        if curr.ant.contained_ant.doubled != True:
                            curr.ant.contained_ant.damage *= 2
                            curr.ant.contained_ant.doubled = True
                    if curr.ant.doubled != True:
                        curr.ant.damage *= 2
                        curr.ant.doubled = True
                curr = curr.exit
...
def apply_effect(effect, bee, duration):
    ...
    action_wrapper = effect(bee.action, bee)
    def new_action(colony):
        nonlocal duration
        if duration != 0:
            action_wrapper(colony)
            duration -= 1
        else:
            old_action(colony)
